    ...
    "itemListElement": [
        @foreach($villas as $index => $villa)
        {
            ...
            "item": {
                ...
                "name": "{{ $villa->name }}",
                "description": "{{ Str::limit($villa->label ?? 'Villa mewah di Dieng', 150) }}",
                "image": "{{ asset('storage/'.$villa->images?->first()?->image ?? '') }}",
...
@if(count($villas) > 0)
    <section class="py-5 lg:py-24 bg-gradient-to-br from-gray-50 to-white" aria-labelledby="popular-villas-heading">
        <div class="max-w-5xl container mx-auto px-4 sm:px-6 lg:px-8">
            <header class="text-center mb-5 lg:mb-12">
                <div class="inline-flex items-center space-x-2 bg-primary-100 text-primary-800 px-3 lg:px-4 py-1 lg:py-2 rounded-full mb-2 lg:mb-4">
                    ...
                    <span class="text-xs lg:text-sm font-medium">Villa Populer</span>
                </div>
                ...
                <p class="text-xs lg:text-lg text-gray-600 max-w-2xl mx-auto">
                    Temukan Villa paling diminati dengan fasilitas lengkap dan harga terbaik
                </p>
